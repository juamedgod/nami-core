...
class BrowserStep extends ExtendedObject {
  ...
  exec(browser, previousPromise) {
    // Deding on the type, data will contain a different spec (a button, a pair of key-value...)
...
class Browser extends ExtendedObject {
  constructor(options) {
    ...
    this.defineProperty('_url');
    this.defineProperty('_initOpts', {initialValue: {}});
    this.defineProperty('xmlHttpRequestTimeout', {initialValue: 120000});
    ...
    _.extend(this._initOpts, options);
    this.autoExec = options.autoExec;

    _.each([
      'assert', 'window', 'history', 'status', 'success', 'statusCode'
    ], (prop) => {
  ...
  reset() {
    ...
    const _this = this;
    this._zombieBrowser = new ZombieBrowser();
    this.xmlHttpRequestTimeout = this._initOpts.xmlHttpRequestTimeout;
    this.site = this._initOpts.site;
    this._zombieBrowser.waitDuration = this._initOpts.waitDuration;
    // zombie does not define form-related attributes in the document
    // so some web apps fail
    this._zombieBrowser.on('loaded', function(document) {
    ...
    this._zombieBrowser.on('opened', function(window) {
  ...
  _parseSelect(item, options) {
    options = _.opts(options, {onlySelected: false});
    const res = [];
